...
#             getOption("repos"))
# if (length(pkgs)>0) 
#     install.packages(pkgs,repos="https://cran.us.r-project.org", type="source")
ipk = function(pkg) {
...
libs = c("lme4","glmmADMB","sos","blme","RLRsim","ggplot2", "MEMSS")
ipk(libs)
...
- you might have *censored* data where all values below a certain limit (e.g. a detection limit) are recorded as zero; in this case you might be able to use `survreg()` and `frailty()` in the `survival` package for random-intercept models (as [suggested on r-help by Thomas Lumley in 2003](https://stat.ethz.ch/pipermail/r-help/2003-June/034905.html) or [on StackOverflow by user 42- in 2014](https://stackoverflow.com/questions/25317021/interaction-terms-and-random-effects-in-tobit-regression-model-in-r). The [lmec package](https://CRAN.R-project.org/package=lmec) handles *linear* mixed models.
...
```
As of `lme4` version 1.1-19, this functionality is available as `isSingular(model)`.
